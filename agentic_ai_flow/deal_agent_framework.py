...
import chromadb
from agent_utils import planning_agent, data_model
...
class DealAgentFramework:
    ...
    def init_agents_as_needed(self):
        if not self.planner:
            ...
            self.planner = planning_agent.PlanningAgent(self.collection)
            self.log("Agent Framework is ready")

    def read_memory(self) -> List[data_model.Opportunity]:
        if os.path.exists(self.MEMORY_FILENAME):
            with open(self.MEMORY_FILENAME, "r") as file:
                data = json.load(file)
            opportunities = [data_model.Opportunity(**item) for item in data]
    ...
    def run(self) -> List[data_model.Opportunity]:
